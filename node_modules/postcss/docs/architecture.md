...
<img width="300" src="https:.wikimedia.org/wikipedia/commons/thumb/a/aa/PostCSS_scheme.svg/512px-PostCSS_scheme.svg.png" alt="workflow">
...
That being said, there are few ways to write parser
 ...
 - *Write a single file with string to AST transformation*
    ...
    This method is quite popular, for example, the [Rework analyzer](https:.com/reworkcss/css/blob/master/lib/parse/index.js) was written in this style. But with a large code base, the code becomes hard to read and pretty slow.
 ...
 - *Split it into lexical analysis/parsing steps (source string → tokens → AST)*
    ...
    A lot of parsers like [`Babylon` (parser behind Babel)](https:.com/babel/babel/tree/master/packages/babylon), [`CSSTree`](https:.com/csstree/csstree) were written in such way.
...
### Core Structures
 ...
 - #### Tokenizer ( [lib/tokenize.es6](https:.com/postcss/postcss/blob/master/lib/tokenize.es6) )
    ...
    const token = [
        ...
        .
        ,
        `space` don't have such information.

        .
        1, 1,

        . Numbers follow same rule as was described above
        1, 10
    ];
    ```
   There are many patterns how tokenization could be done, PostCSS motto is performance and simplicity. Tokenization is complex computing operation and take large amount of syntax analysis time ( ~90% ), that why PostCSS' Tokenizer looks dirty but it was optimized for speed. Any high-level constructs like classes could dramatically slow down tokenizer.
    ...
    PostCSS' Tokenizer use some sort of streaming/chaining API where you exposes [`nextToken()`](https:.com/postcss/postcss/blob/master/lib/tokenize.es6#L48-L308) method to Parser. In this manner we provide clean interface for Parser and reduce memory usage by storing only few tokens and not whole list of tokens.

- #### Parser ( [lib/parse.es6](https:.com/postcss/postcss/blob/master/lib/parse.es6), [lib/parser.es6](https:.com/postcss/postcss/blob/master/lib/parser.es6) )

    Parser is main structure that responsible for [syntax analysis](https:.wikipedia.org/wiki/Parsing) of incoming CSS. Parser produces structure called [Abstract Syntax Tree (AST)](https:.wikipedia.org/wiki/Abstract_syntax_tree) that could then be transformed by plugins later on.
    ...
    There are multiple Node types that PostCSS could produce but all of them inherit from base Node [class](https:.com/postcss/postcss/blob/master/lib/node.es6#L34).

- #### Processor ( [lib/processor.es6](https:.com/postcss/postcss/blob/master/lib/processor.es6) )

    Processor is a very plain structure that initializes plugins and run syntax transformations. Plugin is just a function registered with [postcss.plugin](https:.com/postcss/postcss/blob/master/lib/postcss.es6#L109) call.

    It exposes quite few public API methods. Description of them could be found on [api.postcss.org/Processor](http:.postcss.org/Processor.html)

- #### Stringifier ( [lib/stringify.es6](https:.com/postcss/postcss/blob/master/lib/stringify.es6), [lib/stringifier.es6](https:.com/postcss/postcss/blob/master/lib/stringifier.es6) )
    ...
    The most essential method is [`Stringifier.stringify`](https:.com/postcss/postcss/blob/master/lib/stringifier.es6#L25-L27)
    that accepts initial Node and semicolon indicator.
    You can learn more by checking [stringifier.es6](https:.com/postcss/postcss/blob/master/lib/stringifier.es6)

### API Reference

More descriptive API documentation could be found [here](http:.postcss.org/)
