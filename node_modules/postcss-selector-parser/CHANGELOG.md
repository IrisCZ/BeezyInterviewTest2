...
* `.a   .b`: A comment at the end of the combinator causes extra space to become after space.
  - Old & Busted:
    - `combinator.value === "   "`
...
Because they've been taken off the standardization track, there is no spec-official name for combinators of the form `/<ident>/`. However, I talked to [Tab Atkins](https:.com/tabatkins) and we agreed to call them "named combinators" so now they are called that.
...
= selectorParser.astSync(".a > .b").atPosition(1,4);
combinator.toString() === " > ";
.
...
* **Mutating values with escapes** - When you make an update to a node property that has escape handling
  ...
  is involved. See https:.com/postcss/postcss-selector-parser/pull/133 for an extensive
...
= selectorParser.className({value: "illegal class name", raws: {value: "illegal\\ class\\ name"}});
...
-ascii are escaped to ensure it works in every css file.
className.value = "😱🦄😍";
className.value; => '😱🦄😍'
className.toString(); => '.\\1F631\\1F984\\1F60D'

, or do bad bad things
...
attr.toString(); => "[id='a-value']"
attr.quoted; => true
, only the quotes have changed.
attr.value; => a-value
attr.getQuotedValue(); => "'a-value'";

, no warning because there's no escapes
attr.value = "new-value";
.getQuotedValue(); => "new-value";
...
, it preserves quote mark even when it's not a great idea
...
.setValue('a "double quoted" value', {smart: true, preferCurrentQuoteMark: true, quoteMark: "'"});
=> "[id='a "double quoted" value']"

. Even things that are a bad idea and illegal.
